...
def main():
    ...
    st.markdown("### 📥 Importar Arquivos", help="Carregue os arquivos Excel para visualizar o painel.")
    col1, col2 = st.columns([1, 1])
    with col1:
        checklist_file = st.file_uploader("Checklist", type="xlsx", label_visibility="collapsed")
    with col2:
        manut_file = st.file_uploader("MANU.PREVENT", type="xlsx", label_visibility="collapsed")
    ...
    motorista_opcoes = ["Todos"] + motoristas
    placa_opcoes = ["Todos"] + placas

    sel_motorista = st.sidebar.multiselect("Motoristas", motorista_opcoes, default=["Todos"])
    sel_placa = st.sidebar.multiselect("Placas", placa_opcoes, default=["Todos"])

    if "Todos" not in sel_motorista:
        df = df[df["Motorista"].isin(sel_motorista)]

    if "Todos" not in sel_placa:
    ...
    with aba1:
        # (módulo Visão Geral permanece igual)
        pass

    with aba2:
        # (módulo de Manutenção permanece igual)
        pass

    with aba3:
        # (módulo de Itens Críticos permanece igual)
        pass

    with aba4:
        # (módulo de Observações permanece igual)
        pass
