...
def get_drive_direct_link(url):
    # Tenta converter link do Google Drive para link direto de imagem
...
if uploaded_file_checklist is not None and uploaded_file_manut is not None:
    ...
    # Itens de checklist - excluir colunas de fotos e observações
    ...
    motorista_freq = df["Motorista"].value_counts().idxmax() if not df["Motorista"].empty else "N/A"

    # Abas
    aba1, aba2, aba3, aba4, aba5 = st.tabs([
        "KPIs",
        "Reincidências",
        "Manutenção x Reincidência",
